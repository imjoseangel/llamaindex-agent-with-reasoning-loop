...
def create_agent(uploaded_pdf, _):
...
with gr.Blocks() as demo:
    ...
    output_box = gr.Textbox(autoscroll=True, max_lines=30,
                            value="Output", label='Assistant')
    gr.Interface(fn=response_generator, inputs=[
                 input_box, state_var], outputs=[output_box, state_var])
    upload_button.upload(create_agent, inputs=[upload_button, state_var], outputs=[
                         input_box, state_var], queue=True, show_progress=True, trigger_mode="once")
